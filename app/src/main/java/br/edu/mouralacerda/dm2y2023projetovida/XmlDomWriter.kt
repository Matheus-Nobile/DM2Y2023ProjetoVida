...
fun createGpxDocument(movimentosData: MutableList<Movimento>, activityName: String): String {
    ...
    for (movimento in movimentosData) {
        ...
        trkpt.setAttribute("atividade", "${movimento.atividade}")
        trkpt.setAttribute("nome", "${movimento.atleta}")
...
//    transformer.transform(DOMSource(doc), StreamResult(File("C:\\Program Files\\xml-test-output.xml")))
